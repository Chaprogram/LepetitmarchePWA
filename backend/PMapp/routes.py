...
def reservation():
    if request.method == 'POST':
        ...
        order_details = "; ".join(commandes)  # Convertir la liste en chaîne de caractères pour l'enregistrer
        reservation = Reservation(
            name=name,
            email_reservation=email,
            phone_number=phone,
            order_details=order_details  # Enregistrer les détails de la commande sous forme de chaîne
        )
        db.session.add(reservation)
        db.session.commit()

        # Passer les commandes et total à la page de confirmation
        return redirect(url_for('main.reservation_submit', name=name, email=email, phone=phone, order_details=order_details, total_price=total_price))

    # Si méthode GET, afficher la page de réservation
    return render_template('reservation.html')

@main.route('/reservation_submit', methods=['GET'])
def reservation_submit():
    # Récupérer les données envoyées lors de la redirection
    ...
    return render_template('reservation_submit.html', name=name, email=email, phone=phone, order_details=order_details, total_price=total_price)
